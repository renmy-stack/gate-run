...
function drawWorld() {
  ...
  for (let k = GR.STEPS; k >= 1; k--) {
    ...
    // 上った段には、その段で使った人が並んで残る（絵は 1 列ぶんまで）
    if (k <= climbStep() && k < GR.STEPS + 1) {
      const c = Math.min(GR.stairCost(k) - GR.stairCost(k - 1), 14);
      for (let i = 0; i < c; i++) {
...
function drawStep(k, z1) {
  ...
  ctx.fillStyle = 'rgba(255,255,255,.9)'; ctx.fillText((GR.stairCost(k) - GR.stairCost(k - 1)) + '人', r.x, r.y);
}
